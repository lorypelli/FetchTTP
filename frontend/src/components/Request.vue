...
import { ElButton, ElCheckbox, ElInput, ElTabPane, ElTabs, ElForm, ElFormItem, ElDatePicker, ElMessageBox, ElSelect, ElOption } from 'element-plus';
...
<template>
  <ElTabs class="pr-2">
    <ElTabPane
      :label="`Headers (${headers.filter((h) => {
        return h.enabled
      }).length})`"
    >
      <div
      ...
      >
        ...
        />
        <ElButton @click="add('header', index)">
          +
        </ElButton>
        <ElButton @click="remove('header', index)">
          -
        </ElButton>
      </div>
    </ElTabPane>
    <ElTabPane
      :label="`Query (${query.filter((q) => {
        return q.enabled
      }).length})`"
    >
      <div
      ...
      >
        ...
        />
        <ElButton @click="add('query', index)">
          +
        </ElButton>
        <ElButton @click="remove('query', index)">
          -
        </ElButton>
      </div>
    </ElTabPane>
